...
final readonly class AgentRunController
{
    ...
    private function serializeSummary(AgentRun $run): array
    {
        return [
            ...
            'cache_read_tokens' => $run->getCacheReadTokens(),
            'cache_creation_tokens' => $run->getCacheCreationTokens(),
            'llm_calls' => $run->getLlmCalls(),
            'llm_duration_ms' => $run->getLlmDurationMs(),
            'llm_ttft_ms' => $run->getLlmTtftMs(),
