...
final readonly class UsageQueryService
{
    ...
    {
        $ids = $this->existingIds('attributes', self::dedupe($ids));
        if ([] === $ids) {
            return [];
        }
        ...
        foreach ($ids as $id) {
            ...
            $payload = $this->modelingCache->get(
                ...
                function (ItemInterface $item) use ($id, $ids, &$batch): array {
                    ...
                    $batch ??= $this->loadAttributeUsageBatch($ids);
    ...
    {
        $ids = $this->existingIds('attribute_groups', self::dedupe($ids));
        if ([] === $ids) {
            return [];
        }
        ...
        foreach ($ids as $id) {
            ...
            $payload = $this->modelingCache->get(
                ...
                function (ItemInterface $item) use ($id, $ids, &$batch): array {
                    ...
                    $batch ??= $this->loadAttributeGroupUsageBatch($ids);
    ...
    {
        $ids = $this->existingIds('object_types', self::dedupe($ids));
        if ([] === $ids) {
            return [];
        }
        ...
        foreach ($ids as $id) {
            ...
            $payload = $this->modelingCache->get(
                ...
                function (ItemInterface $item) use ($id, $ids, &$batch): array {
                    ...
                    $batch ??= $this->loadObjectTypeUsageBatch($ids);
     ...
     * }>
     */
    private function loadAttributeUsageBatch(array $ids): array
    {
        ...
        foreach ($ids as $id) {
            $out[$id] = [
                'groups' => $this->normalizeGroupRows($groups[$id] ?? []),
                'objectTypes' => $this->normalizeObjectTypeRows($objectTypes[$id] ?? []),
                'categories' => $this->normalizeCategoryRows($categories[$id] ?? []),
     ...
     * }>
     */
    private function loadAttributeGroupUsageBatch(array $ids): array
    {
        $objectTypes = self::bucketBy($this->fetchByIds(
        ...
        foreach ($ids as $id) {
            $out[$id] = [
                'directlyAttachedTo' => [
                    'objectTypes' => $this->normalizeObjectTypeRows($objectTypes[$id] ?? []),
                    'categories' => $this->normalizeCategoryAttachmentRows($categories[$id] ?? []),
                ],
     ...
     * }>
     */
    private function loadObjectTypeUsageBatch(array $ids): array
    {
        $instanceCounts = self::bucketCounts($this->fetchByIds(
     ...
     * @return list<string>
     */
    private static function dedupe(array $ids): array
    {
