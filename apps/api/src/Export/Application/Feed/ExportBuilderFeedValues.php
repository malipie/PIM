...
namespace App\Export\Application\Feed;
...
/**
 * Export-core adapter that streams a feed's product values through the existing
 * {@see ExportBuilder} (ADR-0023 §6.3, XMLF-P3-02). It lives in Export core (not
 * the Feed sub-area) so it can reuse the catalog id-resolution + value builder
 * the way {@see \App\Export\Application\Sync\SyncExportRunner} does; the Feed
 * generator depends only on the {@see FeedProductValues} seam.
 *
 * The feed's scope becomes an in-memory {@see ExportSession} (never persisted);
 * products are paged in CLEAR_INTERVAL keyset chunks with EntityManager::clear()
 * between pages, so a 50k feed stays memory-flat. Column keys are requested both
 * bare and locale-scoped, then collapsed back to attribute codes for the mapper.
 */
final class ExportBuilderFeedValues implements FeedProductValues
{
    ...
    public function forScope(FeedProductScope $scope): iterable
    {
        ...
        if (null === $tenant) {
            throw new RuntimeException('Feed generation requires a tenant context.');
        }
     ...
     */
    private function session(FeedProductScope $scope, array $columns): ExportSession
    {
     ...
     * @return list<string>
     */
    private function columnKeys(FeedProductScope $scope): array
    {
        $keys = [];
     ...
     */
    private function toAttributeMap(array $row, FeedProductScope $scope): array
    {
        $item = [];
        foreach ($scope->attributeCodes as $code) {
     ...
     * @return list<string>
     */
    private function filterIds(FeedProductScope $scope, Tenant $tenant, ObjectType $objectType): array
    {
        ...
        if (null === $whereClause) {
            throw new RuntimeException('Invalid filter DSL in feed scope.');
        }
