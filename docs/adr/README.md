...
- [0036-platform-instance-and-provisioning.md](0036-platform-instance-and-provisioning.md) — panel operatora przenosi się do osobnej instancji platformowej (`admin.app.harmonpim.pl`), znika z instancji tenantów (404 na routingu); rejestr tenantów w bazie platformy; Dockera dotyka wyłącznie osobna usługa `provisioner`, API nigdy
- [0037-wildcard-certificate-for-tenant-subdomains.md](0037-wildcard-certificate-for-tenant-subdomains.md) — jeden certyfikat wildcard `*.app.harmonpim.pl` wyzwaniem DNS-01 (`caddy-dns/cloudflare`) zamiast certyfikatów per host z `on_demand_tls`; nowy tenant osiągalny bez wpisu w konfiguracji edge i bez przeładowania, endpoint `ask` nie powstaje; ceną jest token Cloudflare w środowisku edge i jeden punkt awarii odnowienia dla wszystkich instancji
- [0038-shared-redis-metrics-registry.md](0038-shared-redis-metrics-registry.md) — countery i histogramy wszystkich workerów trafiają do atomowego registry Redis; restart workera nie zeruje serii, a gauge procesu pozostają świadomie lokalne
